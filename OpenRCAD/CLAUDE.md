...
- `crates/`: The Rust workspace implementing the kernel:
  ...
  - `openrcad-algo`: Boolean operations, fillets, chamfers, offsets, sewing, prism/revolve sweeps, skin (loft) sweeps, plane splitting, healing, corner networks, and the `*_operation` / `*_with_policy` / `*_with_cancel` operation-contract layer.
  - `openrcad-mesh`: B-Rep to triangle mesh tessellation (+ `GpuMesh` render buffers).
  - `openrcad-exchange`: STEP (AP242) read/write, STL write, 3MF write (incl. assembly models).
  - `openrcad-sketch`: 2D sketches / closed profiles, analytic planar arrangement, analytic offsets.
  - `openrcad-document`: parametric document history (sketches + features + recompute) and the `.zcad` document format.
  - `openrcad-render`: interactive `wgpu`/`winit` viewer (orbit/pan/zoom, MSAA, edge wireframe, click-select) plus an embeddable `RenderCore` for host apps (e.g. ZeroCAD's eframe viewport). **Not** in the `openrcad` facade — GPU deps stay out of the kernel.
  - `openrcad-truck-compat`: bidirectional `truck_topology` interop (separate crate, not a `topo` feature).
- `tests/`: cross-cutting facade integration gate (`strict_representation_gate.rs`).

## Reference Guidelines
- When implementing any feature, cross-reference upstream OpenCASCADE C++ logic and the idiomatic Rust patterns in `truck` **if you have those sources available locally** — they are not part of this checkout (design references only; this repo contains no OCCT or truck source).
...
- **Minimal dependencies** — Only `serde` (with `derive`) is allowed as an external dependency for the core kernel crates. Performance crates like `slotmap` and `rayon` are permitted in `openrcad-topo` and `openrcad-mesh` respectively. Known deliberate exceptions outside the kernel core: `openrcad-document` also uses `serde_json`, and `openrcad-render` carries the `wgpu`/`winit` stack (it sits outside the facade). No C/C++ dependencies anywhere.
...
**OpenRCAD's approach:**
- Implement atomic **Euler operators** (`MEV`, `MEF`, `KEV`, `KEF` — the four currently implemented; `MEKR`/`KEMR` are not) that surgically modify the B-Rep graph in the local neighborhood of an edit.
...
- **Step 1: Mutable Topological Staging (`openrcad-topo`)** - Completed `BRepBuilder` in [builder.rs](crates/openrcad-topo/src/builder.rs) supporting edge-splitting and face-partitioning via 2D Jordan curve point-in-polygon containment.
- **Step 2: Intersection & BVH** - Completed `overlapping_pairs` BVH dual-tree traversal in [bvh.rs](crates/openrcad-algo/src/bvh.rs) and `ray_face` intersections on trimmed faces in [intersect.rs](crates/openrcad-algo/src/intersect.rs).
- **Step 3: Gap Healing & Sewing (`openrcad-algo`)** - Implemented tolerant `sew` in [sew.rs](crates/openrcad-algo/src/sew.rs) to merge vertices within a custom tolerance and clean up redundant topological entities.
- **Step 4: Boolean Engine (`openrcad-algo`)** - Implemented the full B-Rep Boolean engine in [boolean.rs](crates/openrcad-algo/src/boolean.rs) supporting Union, Intersection, and Difference:
...
**Current Status & Implementation Details:**
- **Validation (`openrcad-topo`)** — [validate.rs](crates/openrcad-topo/src/validate.rs) adds `Solid::manifold_report` / `is_watertight` / `is_manifold`: it matches undirected boundary edges by quantized endpoint position (independent arena edges at one location merge) and counts face usage — a closed two-manifold solid shares every edge exactly twice. `health_report` warns on non-manifold edges. Boolean integration tests now assert `is_watertight()` on their output (box∩box, box∪box, box−box, and the cylinder-drilled box all pass).
- **Sewing (`openrcad-algo`)** — [sew.rs](crates/openrcad-algo/src/sew.rs) merges coincident vertices/edges within per-entity tolerance and re-threads each loop into a contiguous, consistently-oriented co-edge chain (see Phase 3 Step 6), so sewn results pass `validate()` and `is_healthy()` rather than just `is_watertight()`.
- **Viewer (`openrcad-render`)** — interactive: orbit/pan/zoom, click-to-select with a shader face highlight, **4× MSAA**, and a **topological-edge wireframe overlay** (edges between triangles of different source `face_id`s + open boundaries, extracted in [edges.rs](crates/openrcad-render/src/edges.rs), drawn with a depth-biased line pipeline). Still no materials/multi-object scene or photoreal path.
...
- **Geometry (`openrcad-geom`)** — `RuledSurface` ([ruled.rs](crates/openrcad-geom/src/ruled.rs)), `GregorySurface` (4-sided, G1 boundary, [gregory.rs](crates/openrcad-geom/src/gregory.rs)), `OffsetSurface` ([offset.rs](crates/openrcad-geom/src/offset.rs)), and `ToroidalSurface` ([torus.rs](crates/openrcad-geom/src/torus.rs)) are `GeomSurface` enum variants with `point`/`d1`/`bounds`/`transformed` delegation, each with a point+derivative unit test. The torus is also wired through `interval_bounds`, the BVH, intersection `uv_of`, mesh tessellation, and STEP read/write (`TOROIDAL_SURFACE`).
- **Blend API** — `fillet`, `chamfer`, and `shell_solid` return `Result<Solid, BlendError>`; Shell preserves typed concave-envelope failures through `BlendError::ConcaveShell`. Shared detection + cylinder builders live in [blend.rs](crates/openrcad-algo/src/blend.rs); the box paths stay in fillet/chamfer/offset.
...
- **Shelling** — `shell_solid` retains the box/cylinder fast paths and also accepts the verified analytic, topology-preserving network, including concave planar prisms. Concave candidates require source-to-offset-cell mapping, material classification, exact nonadjacent-pair intersection checks, thickness evidence, and complete history before commit; branching envelopes reject atomically with a typed diagnostic.
- **General rolling-ball sweep (`openrcad-algo/rolling_ball.rs`)** — `rolling_ball_fillet_edge`, `fillet_planar_edge`, `rolling_ball_between_planar_faces`, and `fillet_edges` are now public via the `openrcad` prelude. Handles planar–planar, planar–cylindrical, planar–analytic, and (via `rolling_ball_between_curved_faces`) curved–curved edge adjacency with a Gauss-Newton solver; tangent-edge-chain solvers (`fillet_tangent_edge_chain` / `chamfer_tangent_edge_chain`) also exist. Emits `RollingBallError` variants (`InvalidTolerancePolicy`, `InvalidRadius`, `DegenerateSpine`, `EdgeAdjacency`, `UnsolvableAdjacency`, `InvalidDihedral`, `SpineNotOnFace`, `UnsupportedTrimTopology`, `NewtonDiverged`, `BlendSurfaceBuild`, `InvalidTopology`, `CandidateValidation`, `CornerNetwork`, `Overflow`) for unsupported or degenerate inputs. **`fillet_edges` works on boolean results** — the boolean's collinear-edge merge (Phase 3 Step 6) restores full-span edges, so endpoint-based edge selection finds them instead of failing with `SpineNotOnFace`. `fillet_planar_edge` now also rejects a blend that didn't close (radius larger than half the local thickness → degenerate edges) with `InvalidRadius` instead of returning a broken solid. **`planar_blend` refactor (2026-06-21):** `rolling_ball_fillet_edge` and `rolling_ball_between_planar_faces` share a factored-out `planar_blend(edge, face_a, face_b, n_a, n_b, radius)` core that takes already-outward normals explicitly. This relies on `sew` canonicalizing every sewn shell so each planar face's stored orientation/winding agrees with an outward normal — so the fillet reads `planar_outward_normal` directly (no solid-interior re-derivation) and a sewn prism cap with a once-inward stored orientation no longer mis-seeds the bisector side (pairs with the prism cap-orientation fix below).
- **Selected-edge blend façade (`openrcad-algo/contour.rs`)** — `apply_blend_contour(&solid, &BlendContour)` is a deliberately thin public façade over the per-edge solvers: it routes a `BlendContour { edges, kind: BlendKind::{Fillet|Chamfer}, law: BlendLaw::Constant(_), curve_hint: Option<BlendCurveHint::{Line|Circle}> }` to `fillet_edges`, `fillet_circular_edge_chain` (for a co-circular contour), or `chamfer_edges`, and returns `Result<Solid, BlendContourError>` (`EmptyContour`, `VariableLawUnsupported`, `IncompatibleCornerMode`, `Fillet(..)`, `Chamfer(..)`). `BlendLaw::Variable` is a reserved placeholder. These types plus `RollingBallError`/`ChamferError` are re-exported from `lib.rs`, so a consumer that calls the per-edge API imports them directly. `apply_blend_network_with_policy` blends a whole network of contours with `BlendCornerMode::{Auto, Miter, RollingBall, Setback}` corners.
...
- **Known gaps / next work:** the primitive `fillet`/`chamfer` builders still recognise only a single box or cylinder, while the strict all-edge adapters preserve those fast paths and use the arbitrary-solid selected-edge engines atomically. Shell additionally accepts the verified topology-preserving analytic network, including concave planar prisms; branching nonadjacent intersections and arbitrary NURBS shells remain typed rejections. Equal-radius convex planar corner networks are verified for valences three through six, and contact overflow is verified across successive planar prism faces; mixed-radius/non-planar corner networks and curved-support overflow remain typed rejections. A boolean cut that *severs* a body is returned as one solid (Euler=4); use `Solid::split_disconnected` to get one solid per connected component (the `boolean_bodies` / `boolean_checked_bodies` wrappers are deprecated).
...
### Post-Phase-5 additions (status as of 2026-08-24)

Beyond the five phases above, the kernel has since grown — tracked in detail in
[KERNEL_HARDENING_PLAN.md](KERNEL_HARDENING_PLAN.md):

- **Operation contract layer** — `OperationResult<T>` (value + topology history +
  diagnostics + recovery/validation reports, defined in
  `openrcad-topo/src/operation.rs`), explicit `TolerancePolicy`/`ToleranceContext`
  (`openrcad-foundation/src/tolerance.rs`), and cooperative cancellation
  (`openrcad-foundation` cancellation tokens). The legacy wrappers (`boolean`,
  `boolean_checked`, `fillet`, `chamfer`, `shell_solid`, `prism`, `tessellate`,
  `read_step`, `sew`) are `#[deprecated]` in favor of `*_operation` /
  `*_with_policy` / `*_with_cancel`.
- **New modeling operations** — `revolve.rs` (solids of revolution), `skin.rs` /
  `smooth_skin.rs` (loft/sweep skinning incl. an exact analytic smooth mode),
  `plane_split.rs` (solid splitting), `heal.rs` (solid healing), and the
  `facade.rs` `SolidExt` fluent API + `openrcad::prelude`.
- **Corner networks** (`corner_network.rs`) — exact common rolling-sphere
  solution for equal-radius planar corners, verified for valences 3–6, wired
  into `fillet_edges` via `RollingBallError::CornerNetwork`.
- **Helix curve** in `openrcad-geom`; double-double arithmetic in `openrcad-foundation`.
- **3MF export** in `openrcad-exchange` (incl. multi-definition assembly models);
  **sketch arrangement + analytic offset** in `openrcad-sketch`.
- **Embeddable render core** (`openrcad-render/src/core.rs`) — the GPU scene can
  be driven inside a host app's wgpu device (ZeroCAD's eframe viewport) instead
  of only opening its own window, with incremental instance-buffer uploads.
- Root integration gate: `tests/strict_representation_gate.rs` exercises the
  strict `*_with_policy` representation path end to end.
