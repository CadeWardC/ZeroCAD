...
- **Blend API** — `fillet`, `chamfer`, and `shell_solid` return `Result<Solid, BlendError>`; Shell preserves typed concave-envelope failures through `BlendError::ConcaveShell`. Shared detection + cylinder builders live in [blend.rs](file:///c:/Users/cadel/Documents/Coding_Projects/OpenRCAD/crates/openrcad-algo/src/blend.rs); the box paths stay in fillet/chamfer/offset.
