...
- **Known gaps / next work:** the primitive `fillet`/`chamfer`/`shell_solid` builders still recognise only a single box or cylinder, while the strict all-edge adapters preserve those fast paths and use the arbitrary-solid selected-edge engines atomically. Equal-radius convex planar corner networks are verified for valences three through six, and contact overflow is verified across successive planar prism faces; mixed-radius/non-planar corner networks and curved-support overflow remain typed rejections. Concave-offset self-intersection resolution is not yet implemented. A boolean cut that *severs* a body is returned by `boolean` as one solid (Euler=4); use `boolean_bodies` / `boolean_checked_bodies` (or `Solid::split_disconnected`) to get one solid per connected component.
