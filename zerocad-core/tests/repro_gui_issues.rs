...
use zerocad_core::{
    CornerKind, CoordinateSystem, EdgeRef, ExtrudeMode, FeatureNode, FeatureType, ParametricGraph,
    SketchCurves, Vec3,
};
...
fn extruded_box_fillet_succeeds_and_is_fast() {
    ...
    assert!(warnings.is_empty(), "extruded-box edge fillet should succeed, got {warnings:?}");
    ...
    let has_round = bodies[0].1.vertices.chunks(6).any(|v| {
        v[2] > 11.5 && v[2] < 14.8 && v[1] > 0.05 && v[1] < 4.0
    });
    assert!(has_round, "fillet must produce a rolled surface between floor and top");

    let cracks = total_mesh_cracks(&g);
    assert_eq!(cracks, 0, "filleted extruded box mesh has {cracks} crack edges");
    ...
    assert!(ms < 1500, "single fillet evaluate took {ms} ms — too slow for a live UI-thread preview");
}
...
fn box_can_be_cut_twice() {
    ...
    add_sketch(&mut g, "sketch_3", top_plane(15.0), rect_sketch((5.0, 5.0), (11.0, 11.0)));
    ...
    add_sketch(&mut g, "sketch_5", top_plane(15.0), rect_sketch((29.0, 19.0), (35.0, 25.0)));
    ...
    assert!(warnings.is_empty(), "second cut should also remove material cleanly, got {warnings:?}");
    ...
    assert!(second_floor, "the SECOND cut must carve its own pocket floor (z≈7 in the far corner)");
    assert!(
...
fn cylinder_boss_join_has_no_inward_faces() {
    ...
    g.add_feature(FeatureNode {
        id: "box_1".into(),
        name: "Box".into(),
        feature: FeatureType::Box { w: 40.0, h: 30.0, d: 15.0 },
    });
    let mut c = SketchCurves::new();
    ...
    println!("boss-join: {} triangles, {disagree} normal/winding-disagreements, vol6={vol6:.1}", m.indices.len() / 3);
    assert_eq!(disagree, 0, "{disagree} triangles' stored normal disagrees with winding → they back-face cull and disappear");
    assert!(vol6 > 0.0, "the boss-join shell is wound inside-out (vol6={vol6})");
    ...
    assert_eq!(boundary, 0, "boss-join mesh has {boundary} crack edges (white lines)");
    assert_eq!(nonmanifold, 0, "boss-join mesh has {nonmanifold} non-manifold edges");
}
...
fn fillet_preview_drag_is_responsive() {
    ...
    add_sketch(&mut base, "sketch_1", top_plane(0.0), rect_sketch((0.0, 0.0), (40.0, 30.0)));
    add_extrude(&mut base, "extrude_2", "sketch_1", 15.0, ExtrudeMode::NewBody);
    ...
    assert!(worst < 1500, "a preview frame took {worst} ms — would freeze the UI");
}

// Several realistic "second cut" scenarios to find the "cut works once" failure.
fn cut_scenario(name: &str, second_cut: impl FnOnce(&mut ParametricGraph)) -> (usize, Vec<String>, f32) {
    let mut g = ParametricGraph::new();
    add_sketch(&mut g, "sketch_1", top_plane(0.0), rect_sketch((0.0, 0.0), (40.0, 30.0)));
    add_extrude(&mut g, "extrude_2", "sketch_1", 15.0, ExtrudeMode::NewBody);
    ...
    println!("CUT[{name}]: bodies={}, warnings={warnings:?}", bodies.len());
...
fn second_cut_scenarios() {
    ...
    let (_, w_a, _) = cut_scenario("far-corner", |g| {
        add_sketch(g, "s5", top_plane(15.0), rect_sketch((29.0, 19.0), (35.0, 25.0)));
    ...
    let (_, w_b, _) = cut_scenario("through-hole", |g| {
        add_sketch(g, "s5", top_plane(15.0), rect_sketch((25.0, 12.0), (31.0, 18.0)));
    ...
    let (_, w_c, _) = cut_scenario("edge-coplanar", |g| {
        add_sketch(g, "s5", top_plane(15.0), rect_sketch((30.0, 0.0), (40.0, 10.0)));
        add_extrude(g, "extrude_6", "s5", -8.0, ExtrudeMode::Cut);
        g.add_dependency("extrude_4", "extrude_6");
    });
    ...
    let (_, w_d, _) = cut_scenario("same-spot-deeper", |g| {
        add_sketch(g, "s5", top_plane(15.0), rect_sketch((5.0, 5.0), (11.0, 11.0)));
        add_extrude(g, "extrude_6", "s5", -14.0, ExtrudeMode::Cut);
        g.add_dependency("extrude_4", "extrude_6");
    });

    let mut failures = vec![];
    for (n, w) in [("far-corner", &w_a), ("through-hole", &w_b), ("edge-coplanar", &w_c), ("same-spot-deeper", &w_d)] {
        if !w.is_empty() {
            failures.push(format!("{n}: {w:?}"));
        }
    }
    assert!(failures.is_empty(), "second-cut failures:\n{}", failures.join("\n"));
}
...
fn cut_with_positive_depth_on_top_face_still_cuts() {
    ...
    add_sketch(&mut g, "sketch_3", top_plane(15.0), rect_sketch((10.0, 10.0), (20.0, 20.0)));
    add_extrude(&mut g, "extrude_4", "sketch_3", 8.0, ExtrudeMode::Cut);
    g.add_dependency("extrude_2", "extrude_4");

    let (bodies, warnings) = g.evaluate_bodies_with_warnings(&HashSet::new()).unwrap();
    assert_eq!(bodies.len(), 1, "positive-depth cut keeps one body");
    assert!(warnings.is_empty(), "the cut should carve cleanly, got {warnings:?}");
    ...
    assert!(floor, "a positive-depth top-face cut must still carve a pocket into the body");
}
