...
fn inspect_live_edge(body: &LiveBody, reference: &EdgeRef) -> Result<EdgeInspection, String> {
    ...
    let curve_kind = match &curve {
        InspectionCurve::Kernel(edge) => edge.curve().map_or("degenerate", |curve| match curve {
            ...
            openrcad::geom::GeomCurve::TorusPlaneSection(_) => "torus-plane section",
            openrcad::geom::GeomCurve::Reparametrized(_) => "reparameterized",
            openrcad::geom::GeomCurve::TorusSurfaceCurve(_) => "torus surface curve",
        }),
